...
from rag.config import RAG_QUERY_REWRITE_ENABLED
...
class ChatService:
    ...
    async def _rewrite_query_for_retrieval(self, question: str) -> str:
        """用 LLM 将用户问题改写为更适合向量检索的查询；失败时退回原文。"""
        q = question.strip()
        if not q:
            return q
        try:
    ...
    async def rag_chat_astream(
        ...
        if RAG_QUERY_REWRITE_ENABLED:
            retrieval_query = await self._rewrite_query_for_retrieval(question)
            if retrieval_query != question.strip():
                logger.info(
                    f"检索查询已改写：{question[:120]!r} -> {retrieval_query[:120]!r}"
                )
            else:
                logger.debug("检索查询改写后与原文一致或未变化")

        logger.debug(f"执行检索，top_k={top_k}")
        try:
            nodes = await query_engine.aretrieve(retrieval_query)
        except Exception:
            nodes = query_engine.retrieve(retrieval_query)
