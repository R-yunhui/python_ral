...
**⚠️ 工具参数禁止写死用户/查询类取值**：
- 凡表示「用户输入、查询条件、筛选条件、地域、时间、关键词」等的参数（如 province、city、district、date、keyword、query、status、name 等），**默认值必须为空字符串 `""`**，或**绑定到输入/上游变量**（如 `{{#input_xxx.user_input#}}`、`{{#code_xxx.city#}}`）
- **禁止**将此类参数写死为具体地名、日期、关键词等（如 "广东"、"深圳"、"北京"、具体日期），否则工作流无法适配用户每次不同的输入
- 仅当参数语义是「固定配置」（如接口版本、每页条数等）时，才可给出具体默认值
...
13. **Tool 节点**: 参数值引用用户输入时使用 `{{#input_xxx.user_input#}}`；`tool_key` 必须与上下文提供的值完全一致（MCP 工具包含 `_数字ID` 后缀）；**用户/查询类参数（省市区、时间、关键词等）禁止写死为具体值，必须留空或绑定变量**
...
- ❌ 工具节点将 province、city、date、keyword 等用户/查询类参数写死为 "广东"、"深圳" 等具体值 → ✅ 此类参数 value 留空 `""` 或绑定到输入/上游变量
