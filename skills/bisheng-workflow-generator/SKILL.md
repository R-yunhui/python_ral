...
**多分支场景**：多个 LLM 分支汇聚到同一个 Output 节点时，`msg` 引用其中一个分支的输出，`varZh` 必须列出所有分支的输出变量（帮助平台识别所有可能的变量引用）。各分支也可以分别接独立的 Output 节点。
...
4. **条件分支**：sourceHandle 使用条件 ID；Condition 节点的出边数 = 条件分支数量
5. **多轮对话**：必须包含 Output→Input 循环边
