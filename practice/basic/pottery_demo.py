...
1. pottery 的 Redlock 是真正的 Redlock 算法（多 master 共识），
   释放时也要验证 quorum，可能抛 ReleaseUnlockedLock（单 Redis 实例时常见）。
   捕获该异常即可，锁本身功能正常。

2. RedisCounter["key"] += 1 不是原子操作（读→加→写三步），
   高并发下会丢失更新。原子计数请用 redis_client.incr()。

3. pottery 不提供信号量，需要自己写（见 redis_semaphore_all.py）。
"""
...
def demo_redlock():
    ...
    │ )                               │                                     │
    │                                 │                                     │
    │ lock.acquire(blocking=False)    │ lock.tryLock();                     │
    │ if got: try: ... finally: ...   │ try { ... } finally { lock.unlock();}│
    └─────────────────────────────────┴─────────────────────────────────────┘

    注意：单 Redis 实例时 release() 可能抛 ReleaseUnlockedLock，
    捕获即可，不影响锁功能。
    """
    print("\n=== 1. 分布式锁 (Redlock) ===")
    ...
    if lock.acquire(blocking=False):
        try:
            ...
            try:
                ...
                print("    锁已过期（ReleaseUnlockedLock），跳过")
    else:
        print("    锁被占用，跳过")

    # --- 用法二：并发场景（非阻塞 + 重试） ---
    print("\n  [用法二] 并发场景（非阻塞 acquire + 重试）")

    def worker(name: str):
        for attempt in range(3):  # 最多重试 3 次
            if lock.acquire(blocking=False):
                try:
                    print(f"    [{name}] 第{attempt+1}次尝试，获取成功！执行任务...")
                    time.sleep(1)
                    print(f"    [{name}] 任务完成")
                finally:
                    try:
                        ...
                        print(f"    [{name}] 锁已过期，跳过")
                return  # 成功执行，退出
            else:
                print(f"    [{name}] 第{attempt+1}次尝试，锁被占用，等待...")
                time.sleep(0.5)
        print(f"    [{name}] 重试耗尽，放弃")
...
def demo_atomic_counter():
    ...
    redis_client.delete("pottery:demo:counter_good:total")
